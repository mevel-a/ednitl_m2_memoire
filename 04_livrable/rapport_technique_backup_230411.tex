...
Les glossaires des concepts adverses et des expressions privilégiées ne sont pas encore pleinement implémentés. D'une part l'encodage n'est pas terminé et d'autre part, la version actuelle ne liste que les expressions elle-mêmes, dans une version ultérieure l'encodage sera modifié afin d'afficher le contexte des expressions, permettant au lecteur de mieux saisir le sens des expressions (le terme «~vrai~» seul a moins d'intérêt que l'expression «~écrire de "vrai" roman~». %Nous savons cependant déjà comment ils le seront (voir \ref{encW}).
...
\section{Conception et réalisation d'une base de donnée}
    ...
    %Rapide, modifiable à souhait et pratique d'utilisation pour une alimentation continue au fil d'une lecture, la base de données relationnelle nous parût un outil performant pour traiter et mettre en valeur les relations transtextuelles qui parcourent \textit{Pour un nouveau roman} et l'intègrent au sein d'un corpus plus vaste.
