...
Seules les citations font l'objet d'un traitement proprement transtextuel. Si l'objectif initial, et surtout, idéal, de ce travail était de traiter l'intégralité des citations, le temps nécessaire à un tel travail nous imposa de réduire la voilure.



% dire que les statuts référentiels sont gérés dans DB
% les références et quote = infobulle avec les infos de statut
    % pas de propos général sur des références 
         % pas surchargé truc
         % intérêt ?
    % nous posons encore la question d'en ajouter en tant que note explicative
% réduction des quote en context
    % sartre
    % pinget
    % 

% la table des matières pour son versant rhétorique/stylistique, articulation de la pensée, encore en chantier
Nous nous proposions de fournir pour les articles une table des matières qui permettrait de «~piocher~» dans le recueil des passages de la pensée de Robbe-Grillet. Ces renvois ne sont pas encore implémentés mais le seront prochainement (voir \ref{%ENCODAGE
}).
%A DEPLACER
Nous employons des éléments <milestone/>, bornes, pour inscrire les points rhétorique importants. Lors de la transformation XSL ces <milestone> seront transformé en élément <a/>, \textit{anchor} ancre, ces éléments sans contenu seront invisible au lecteur mais permettront de constituer un menu de navigation sur la gauche de l'écran en XSL (voir \ref{%
}). Leur attribut @id construit sur le modèle~: «~refutation2~»
