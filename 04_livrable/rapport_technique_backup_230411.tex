...
\label{ref:db_modele_conceptuel}
 ...
 . Les modèles conceptuels sont constitués~: 
    \begin{itemize}
        ...
        \item d'association, les relations qui unissent les entités (par exemple~: «~correspond à~», «~mentionne~»)
...
Au contraire, lorsque les deux entités sont reliées par une association dont les cardinalités maximales sont «~n~», alors l'association devient une relation contenant %AUTRE CAS
...
Ces fichiers se présentent sous la forme de fichiers «~.html~», un fichier par chapitre du recueil. Grâce à un script perl nous récupérons l'ensemble du contenu textuel et procédons à son nettoyage (nous remplaçons les balises html par des balises xml-tei lorsque cela est pertinent et supprimons toutes les balises inutiles).
%DÉTAIL EN ITEMIZE VOIR NB8NETTOYAGE;TXT

%- nettoyage en conservant grâce à des commentaires les traces de balises supprimées mais qu'on pourrait vouloir restaurer (voir "bloc titre")

Nous obtenons alors un fichier \go.xml\gf pas tout à fait valide mais qui constitue un bon point de départ pour un encodage plus complet.
...
Alors que la majorité des encodages TEI se contente d'un seul élément <text> contenant l'œuvre ou le manuscrit encodé nous employons un élément corpus> qui contiendra plusieurs éléments <text> identifiés grâce à des attributs @xml:id.

Si nous reproduisons l'intégralité de \punr{} dans l'élément <text> qui lui correspond nous n'insérons dans les autres éléments <text> que les extraits qui nous intérèsse~: nous produisons bien une édition de \punr{} inscrit dans un corpus plus vaste, pas l'édition d'un corpus dont \punr{} ne serait qu'un détail. Extraits transtextuels et passages de \punr{} correspondant sont ensuite liés via un jeu d'attributs @corresp et @xml:id.
%screen corresp
...
Une fois l'encodage terminé, l'encodeur concçoit une transformation XSL, soit un fichier contenant des informations de traitement afin de passer d'un fichier XML peu lisible pour le lecteur à une édition numérique au format HTML. En effet les feuilles de styles XSL permettent de conserver le fichier XML originel pour en créer d'autres de types divers, en l'occurence nous nous contentons de produire un site internet, soit des pages au format HTML. Notons que le langage de balisage HTML est un dérivé de l'XML qui ne permet pas de structurer le contenu aussi finement que l'XML mais permet un affichage via navigateur web pour lecture.
    \subsection{Principes généraux}
Les transformations XSL fonctionne par \textit{template}, patron, qui commande le traitement d'un ou de plusieurs éléments XML selon des restrictions diverses laissées au soin de l'auteur de la transformation. On peut par exemple transformer un élément <ab> muni d'un attribut @corresp en un lien hypertexte, qui, lié à des scripts (voir \ref{js}) permettra l'affichage de contenu supplémentaire. 
    \subsection{}
}
\section{Une expérience de lecture~: ajouts de script}
\label{js}
