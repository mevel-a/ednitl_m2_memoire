...
\begin{itemize}
    ...
    \item Enfin Pinget nous a semblé être l'auteur pour lequel Robbe-Grillet réservait le plus d'éloge. La section «~Un roman qui s'invente lui-même~» est situé en fin de l'«~Anthologie~» laissant entendre qu'il s'agit du plus «~modderne~» selon Robbe-Grillet qui écrivait encore dans les années 1990 dans le quotidien \textit{Le Monde} que «~\footnoteB{%REFENRECE
    }~» %REFERENCE MANQUANTe
    . Nous avions ainsi deux des pôles axiologiques les plus éloignés mentionnés explicitement.
...
\verbatim{<span class="ref">contenu sur lequel porte la note<span class="refinfo">la note elle-même</span></span> la suite du contenu}, 
...
\verbatim{contenu sur lequel porte la notela note elle-même la suite du contenu}.
Or nous souhaitons que le second segment ne s'affiche que lorsque le curseur passe sur la souris. C'est ici qu'intervient le css. %figure{screen_cssinfo.png ou truc du genre}

Comme on peut le voir dans \ref{screenCssInfo}, css emploie des \textit{selector} auquel il attribut des propriétés. En l'occurence la \textit{class} «~ref~» est semectionnée ligne 299 et lui est attribué une couleur (vert) ligne 300.
