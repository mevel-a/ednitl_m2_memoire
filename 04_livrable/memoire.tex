...
%%citation
à l'inverse de ces détracteurs dont les positions sont artificielles, construites en dépit d'une réalité palpable
%citation
Les thèses des adversaires de Robbe-Grillet semblent «~malheureusement\footnote{\op}~» devoir être toujours fausses puisqu'elles procèdent par la suimposition au réel de grilles de lecture predéterminées, de clichetsé:
%citation
Au contraire les thèses de Robbe-Grillet sont exposée comme une acceptation de ce principe tel  fréquente introduction par des modalisateurs le souligne~:
%ex citation
...
De même, %ch11 ou 12
prenant le contrepied exprime clairement la méthode (prendre le contre-pied), si le procédé repose sur un argument facile il n'en demeure pas moins juste~: les adversaires ayant tort c'est donc que l'inverse est vrai et \robbe{} ne saurait avoir tort. 
%citattiongoes here
%citattiongoes here
%citattiongoes here
 
Cependant puisqu'il s'agit pour \robbe{} de rétablir les faits contre les préjugés, il ne suffit en réalité pas d'en dire le contraire. C'est là qu'est l'habileté et la distinction entre «~l'exact contre-pied\footnote{\op144}~» et l'«~exactement [...] inverse\footnote{\op143}~»~: les positions adverses résumés portent sur des positions de principes aux contours peu défini mais trop généraux pour ne pas prêter aisément le flan critique~; ainsi il suffit de proposer une position plus ouverte en contre point, pour sembler plus difficilement réfutable.
%citattiongoes here
%citattiongoes here
%
